...
function createFact(operation, left, right) {
  ...
  const ordered = commutative
    ...
    : [left, right];
  const storageSymbol = getOperationStorageSymbol(operation);
  const displaySymbol = getOperationDisplaySymbol(operation);
  return {
    operation,
...
function buildAdditionPool(settings) {
  const selectedDifficulties = Array.isArray(settings.additionDifficulties)
    ? settings.additionDifficulties.filter((value) => ["easy", "medium", "hard"].includes(value))
    : [];
  ...
  const buckets = difficulties.flatMap((difficulty) => ADDITION_DIGIT_BUCKETS[difficulty] || []);
  const allowRegrouping = settings.additionRegrouping !== false;
  ...
  buckets.forEach(([leftDigits, rightDigits]) => {
    ...
    while (created < perBucketTarget && attempts < maxAttempts) {
      ...
      if (!allowRegrouping && hasAdditionRegroupingCore(left, right)) {
        continue;
      }
...
function buildDivisionPool(settings) {
  ...
  if (settings.sessionType === "isolation") {
    const divisor = clampNumber(Number(settings.focusFactor), 1, FACTOR_LIMIT, 1);
    for (let quotient = settings.minFactor; quotient <= settings.maxFactor; quotient += 1) {
      addFactVariant(map, "division", divisor * quotient, divisor);
    }
...
function inferSymbolFromFactKey(key) {
  ...
  return getOperationDisplaySymbol(operation);
}
...
function renderDailyProgress() {
  if (
    !elements.attemptBadge ||
    !elements.accuracyBadge ||
    !elements.attemptProgressLabel ||
    !elements.accuracyProgressLabel
  ) {
    return;
  }
...
function startSession(settings) {
  ...
  state.session = createEmptySession(settings);
  ...
  badgeIcon.className = "session-badge-icon visual-anchor";
  badgeIcon.setAttribute("aria-hidden", "true");
  badgeIcon.textContent = OPERATION_SYMBOLS[settings.operation] || "=";
  const badgeLabel = document.createElement("span");
  badgeLabel.textContent = getSessionBadgeLabel(settings);
  elements.sessionBadge.replaceChildren(badgeIcon, badgeLabel);
...
function appendWorkoutHistory(reason) {
  ...
  const averageMs = average(state.session.responseTimes);
  const medianMs = median(state.session.responseTimes);
  const fastCorrect = (state.progress.answerTelemetry || [])
    .filter((entry) => entry.sessionId === state.session.id && entry.correct && entry.responseTimeMs !== null)
    .filter((entry) => entry.responseTimeMs <= getFluencyThresholdMs(entry.operation))
    .length;
  const record = normaliseWorkoutRecord({
    ...
    additionDifficulties: state.settings.additionDifficulties,
    additionRegrouping: state.settings.additionRegrouping,
  });

  state.progress.workoutHistory = [record, ...state.progress.workoutHistory].slice(0, 50);
}

function getDifficultyBandForQuestion(question) {
  const operation = question?.operation || state.settings?.operation || "multiplication";
  if (operation === "addition") {
    return state.settings?.additionDifficulty || "easy";
  }
  if (operation === "subtraction") {
    return state.settings?.subtractionDifficulty || "easy";
  }
  if (operation === "multiplication" || operation === "division") {
    const left = Math.abs(Number(question?.a ?? question?.left) || 0);
    const right = Math.abs(Number(question?.b ?? question?.right) || 0);
    const maxFactor = Math.max(left, right);
    if (maxFactor <= 5) {
      return "easy";
    }
    if (maxFactor <= 10) {
...
function getFluencyThresholdMs(operation) {
...
function appendAnswerTelemetry(question, evaluation, skipped, responseTimeMs) {
  const operation = question?.operation || state.settings?.operation || "multiplication";
  const timestamp = Date.now();
  const entry = normaliseAnswerTelemetryEntry({
    id: `${timestamp}-${Math.random().toString(36).slice(2, 7)}`,
    sessionId: state.session.id,
    sessionSource: state.session.source,
    sessionPosition: state.session.answered + state.session.skipped,
    operation,
    factKey: question?.key || "",
    skillKey: getLearningSkillKey(question),
    difficultyBand: getDifficultyBandForQuestion(question),
  ...
  state.progress.answerTelemetry = [entry, ...(state.progress.answerTelemetry || [])]
...
function updateFactProgress(question, isCorrect, responseTimeMs) {
  ...
  const updated = {
    ...
    symbol: question.symbol || getOperationDisplaySymbol(question.operation || "multiplication"),
...
function registerAnswer(evaluation, answerValue, options = {}) {
  ...
  if (skipped) {
    ...
    appendAnswerTelemetry(state.currentQuestion, null, true, null);
    ...
    appendAnswerTelemetry(state.currentQuestion, evaluation, false, responseTimeMs);
...
function completeSession(reason = "manual") {
  ...
  showHomeStreakBanner(streakSummary.current);
