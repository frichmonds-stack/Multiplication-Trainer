...
$liveLabel = if ($Label.Trim()) {
  "Live - $($Label.Trim())"
} else {
  "Live - $appVersion current build"
}
...
$snapshotItems = $versionDirs | ForEach-Object {
  $name = $_.Name
...
<body>
  ...
  <h2>Current Live Build</h2>
  <ul>
    <li><a href="./live/">$liveLabel (latest)</a></li>
  </ul>
  <h2>Preserved Snapshots</h2>
  <ul>
$($snapshotItems -join "`r`n")
  </ul>
</body>
</html>
"@
